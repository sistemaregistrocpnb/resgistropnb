window.initModVehiculos = function() {
  ...
  function cargarMarcas(tipo) {
    ...
    Object.keys(lista).sort().forEach(m => {
    ...
    try {
      ...
      if (!errVinc && vinculados && vinculados.length > 0) {
        vinculados.forEach(reg => resultados.push({ 
          ...
          titulo: '🔗 Vinculado (Persona + Vehículo)', 
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const updateData = {
        ...
        foto_frontal: f1, 
        foto_trasera: f2, 
        foto_lado_derecho: f3, 
        foto_lado_izquierdo: f4
      };
