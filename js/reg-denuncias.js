window.initRegPersonas = function() {
    ...
    async function verificarCedula(cedula) {
        ...
        try {
            const { data: dataPersonas } = await window.supabaseClient.from('registro_personas').select('cedula').eq('cedula', cedula).maybeSingle();
            if (dataPersonas) {
            ...
            const { data: dataProc } = await window.supabaseClient.from('registro_procesados').select('cedula, tipo_delito').eq('cedula', cedula).maybeSingle();
            if (dataProc) {
                cedulaStatus.className = 'cedula-status error';
    ...
    form.addEventListener('submit', async (e) => {
        ...
        // ✅ LÓGICA DE TELÉFONOS RESTAURADA Y SEGURA
        ...
        try {
            ...
            const data = {
                ...
                edad,
                tlf_pais: tlfCodigo, 
                tlf_numero: tlfNumero, // ✅ TELÉFONO GUARDADO CORRECTAMENTE
            ...
            await registrarLog('CREAR', 'PERSONAS', {
                cedula: cedula,
