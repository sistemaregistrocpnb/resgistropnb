window.initRegVehiculos = function() {
    ...
    // ✅ FUNCIÓN PARA REGISTRAR LOGS CON NOMBRE COMPLETO (SIN DUPLICADOS)
    ...
    ['v_foto_frontal', 'v_foto_trasera', 'v_foto_der', 'v_foto_izq', 'v_foto_frontal_a', 'v_foto_trasera_a', 'v_foto_der_a', 'v_foto_izq_a'].forEach((id) => {
    ...
    async function checkAvailability(input, msgId) {
        ...
        try {
            let found = false;
            let foundIn = '';
            const col = input.id === 'v_placa' ? 'placa' : input.id === 'v_serial_carroceria' ? 'serial_carroceria' : 'serial_motor';
            ...
            const { data: dataPrincipal } = await window.supabaseClient.from(tablaPrincipal).select('id').ilike(col, val).limit(1);
            if (dataPrincipal && dataPrincipal.length > 0) { found = true; foundIn = tipoVehiculo === 'Motocicleta' ? 'Motocicletas' : 'Automóviles'; }
            
            const { data: dataVinculados } = await window.supabaseClient.from('registro_vinculado').select('id').ilike(col, val).eq('tipo_vehiculo', tipoVehiculo).limit(1);
            if (dataVinculados && dataVinculados.length > 0) { found = true; foundIn = 'Vehículos Vinculados'; }
            
            if (found) {
                input.classList.add('input-error'); input.classList.remove('input-valid');
                if (msgEl) { msgEl.textContent = `❌ Ya registrado en ${foundIn}`; msgEl.className = 'status-msg error'; }
            } else {
    ...
    document.getElementById('v_placa')?.addEventListener('input', debounce((e) => checkAvailability(e.target, 'msg-placa'), 600));
    document.getElementById('v_serial_carroceria')?.addEventListener('input', debounce((e) => checkAvailability(e.target, 'msg-carroceria'), 600));
    document.getElementById('v_serial_motor')?.addEventListener('input', debounce((e) => checkAvailability(e.target, 'msg-motor'), 600));
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // ✅ REGISTRAR EL LOG UNA SOLA VEZ (DUPLICADO ELIMINADO)
