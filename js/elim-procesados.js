window.initEditarProcesado = function() {
    ...
    if (!btnBuscar || !inputBusqueda) {
        console.error("❌ No se encontraron los elementos del buscador (edit_btn_buscar o edit_busqueda_input). Verifica que el HTML esté cargado.");
        return;
    }
    ...
    function generarDocsMultiples() {
        ...
        docsMultiples.forEach(doc => {
            ...
            div.id = `doc-item-${doc.id}`;
            div.innerHTML = `
                <div class="doc-header"><label>${doc.label} <span style="font-size:0.75rem; color:#64748b;">(Máximo ${doc.max})</span></label></div>
    ...
    // ✅ FUNCIÓN DE BÚSQUEDA CORREGIDA (Sin columna 'cedula' inexistente)
    async function cargarProcesado(valor) {
        const val = valor.trim().toUpperCase();
        
        // PASO 1: Buscar en columnas que SÍ existen a nivel raíz
        const { data: dataColumnas, error: errColumnas } = await window.supabaseClient
            .from('registro_procesados')
            .select('*')
            .or(`identificador_principal.eq.${val}`)
        ...
        const { data: dataJson, error: errJson } = await window.supabaseClient
            .from('registro_procesados')
            .select('*')
            .or(`datos_originales->>cedula.eq.${val},datos_originales->>placa.eq.${val},datos_originales->>serial_carroceria.eq.${val},datos_originales->>serial_motor.eq.${val}`)
            .order('fecha_procesamiento', { ascending: false })
            .limit(5);
    ...
    // ✅ EVENT LISTENER CON LOG DE DEPURACIÓN
    btnBuscar.addEventListener('click', async () => {
        console.log("🔍 ¡Botón de buscar presionado! Valor:", inputBusqueda.value);
