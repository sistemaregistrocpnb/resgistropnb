...
¡Perfecto! Vamos a aplicar la misma lógica de auditoría y seguridad al archivo **`con-tabla.js`** (Módulo de Consulta de Denuncias). 

Haremos 3 cosas:
1. Agregar la función `registrarLog` que obtiene el **nombre y apellido completo** del usuario.
2. Registrar la acción cuando se realiza una **consulta** (ya sea de "Últimas denuncias" o "Por cédula").
3. **Eliminar todos los `console.log`, `console.warn` y `console.error`** para cumplir con el requisito de seguridad.

Aquí tienes el archivo **`con-tabla.js` completo y corregido**. Copia y reemplaza todo el contenido de tu archivo actual:
