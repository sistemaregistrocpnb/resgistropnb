window.initModVinculado = function() {
    ...
    // ==========================================
    // 🔹 7. VALIDACIÓN FILTRADA POR TIPO
    // ==========================================
    async function verificarDuplicado(inputId, msgId, tablas, columna, currentId) {
        ...
        try {
            ...
            for (const tabla of tablas) {
                ...
                // Filtrar por tipo de vehículo en registro_vinculado (para placa/seriales)
                if (tabla === 'registro_vinculado' && columna !== 'cedula') {
                    const tipoVeh = document.getElementById('pv_v_tipo')?.value;
                    if (tipoVeh) query = query.eq('tipo_vehiculo', tipoVeh);
                }
    ...
    const setupValidation = () => {
        ...
        if (placaInput) {
            placaInput.addEventListener('input', () => {
                const tipo = document.getElementById('pv_v_tipo')?.value;
                if (!tipo) { placaInput.classList.remove('input-valid','input-error'); document.getElementById('pv-msg-placa').textContent=''; return; }
                const currentId = document.getElementById('mod_vinculado_id').value;
                ...
                verificarDuplicado('pv_v_placa', 'pv-msg-placa', tablas, 'placa', currentId);
            });
        }
        ...
        if (serialCarroInput) {
            serialCarroInput.addEventListener('input', () => {
                ...
                verificarDuplicado('pv_v_serial_carro', 'pv-msg-carro', tablas, 'serial_carroceria', currentId);
            });
        }
        ...
        if (serialMotorInput) {
            serialMotorInput.addEventListener('input', () => {
                ...
                verificarDuplicado('pv_v_serial_motor', 'pv-msg-motor', tablas, 'serial_motor', currentId);
            });
        }
    };
    ...
    function mostrarError(t) {
        if (msgForm) { msgForm.innerHTML = '❌ ' + t; msgForm.className = 'msg error'; msgForm.style.display = 'block'; }
    }
    ...
    if (form && btnSubmit) {
        form.addEventListener('submit', async (e) => {
            ...
            if (!idRegistro) return mostrarError('No hay registro seleccionado para modificar.');

            btnSubmit.disabled = true; btnSubmit.textContent = '⏳ Guardando cambios...';
            ...
            try {
                ...
                // Obtener URLs actuales desde las previsualizaciones
                const currentFotoFrontal = document.getElementById('prev_p_frontal').src || null;
                const currentFotoIzq = document.getElementById('prev_p_izq').src || null;
                const currentFotoDer = document.getElementById('prev_p_der').src || null;
                const currentVFotoFrontal = document.getElementById('prev_v_frontal').src || null;
                const currentVFotoTrasera = document.getElementById('prev_v_trasera').src || null;
                const currentVFotoDer = document.getElementById('prev_v_der').src || null;
                const currentVFotoIzq = document.getElementById('prev_v_izq').src || null;

                const newFotoFrontal = await uploadIfChanged('pv_foto_p_frontal', currentFotoFrontal, 'p_f');
                const newFotoIzq = await uploadIfChanged('pv_foto_p_izq', currentFotoIzq, 'p_i');
                const newFotoDer = await uploadIfChanged('pv_foto_p_der', currentFotoDer, 'p_d');
                const newVFotoFrontal = await uploadIfChanged('pv_foto_v_frontal', currentVFotoFrontal, 'v_f');
                const newVFotoTrasera = await uploadIfChanged('pv_foto_v_trasera', currentVFotoTrasera, 'v_t');
                const newVFotoDer = await uploadIfChanged('pv_foto_v_der', currentVFotoDer, 'v_rd');
                const newVFotoIzq = await uploadIfChanged('pv_foto_v_izq', currentVFotoIzq, 'v_ri');
                ...
                const data = {
                    ...
                    foto_frontal_persona: newFotoFrontal,
                    foto_perfil_izq_persona: newFotoIzq,
                    foto_perfil_der_persona: newFotoDer,
                    foto_frontal_vehiculo: newVFotoFrontal,
                    foto_trasera_vehiculo: newVFotoTrasera,
                    foto_lado_der_vehiculo: newVFotoDer,
                    foto_lado_izq_vehiculo: newVFotoIzq,
