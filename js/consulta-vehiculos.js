...
// ✅ CORRECCIÓN: Usar onclick en lugar de addEventListener
setTimeout(() => {
...
if (!modalBody || !modalTitulo) {
console.error("❌ No se encontró modalBody o modalTitulo");
return;
}
...
// Insertar reporte y obtener consecutivo
...
// Obtener datos de procesados si aplica
...
// ✅ GENERAR HTML CON MEMBRETE COMPLETO
...
{ label: 'Serial de Motor', value: data.serial_motor },
{ label: 'Serial de Carrocería', value: data.serial_carroceria },
{ label: 'Cilindraje', value: data.cilindraje },
{ label: 'Tipo de Carrocería', value: data.tipo_carroceria },
{ label: 'Cédula del Propietario', value: data.cedula_propietario },
...
{ label: 'Fecha de Registro', value: data.creado_en ? new Date(data.creado_en).toLocaleString('es-VE') : null }
];
...
if (data.observaciones) {
html += `<div class="ficha-completa-item full-width"><div class="ficha-completa-label">📝 Observaciones del Vehículo</div><div class="ficha-completa-value">${data.observaciones}</div></div>`;
}
}

html += `</div>`;

// ✅ DATOS DE LA PERSONA (solo para vinculados)
...
{ label: 'Usa Lentes', value: data.usa_lentes !== undefined ? (data.usa_lentes ? 'Sí' : 'No') : null },
{ label: 'Detalle de Lentes', value: data.detalle_lentes },
{ label: 'Tiene Perforaciones', value: data.perforaciones !== undefined ? (data.perforaciones ? 'Sí' : 'No') : null },
{ label: 'Detalle de Perforaciones', value: data.detalle_perforaciones },
...
console.warn('Error cargando incidencias:', err);
}
...
<p style="margin-top: 10px;"><strong>N° de Reporte:</strong> ${numeroReporte}</p>
</div>`;
...
<p>${err.message}</p>
<button onclick="location.reload()" style="margin-top: 20px; padding: 10px 20px; background: var(--primary); color: white; border: none; border-radius: 5px; cursor: pointer;">Recargar Página</button>
</div>`;
}
}
