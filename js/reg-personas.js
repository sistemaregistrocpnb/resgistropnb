...
if (optionsBox && nativeSelect && displayBox) {
    ...
    Array.from(nativeSelect.options).forEach(opt => {
        ...
        // ✅ AQUÍ ESTÁ LA CLAVE: Al hacer clic, actualizamos el select oculto
        div.addEventListener('click', () => {
            nativeSelect.value = opt.value; // Actualiza el valor real del formulario
    ...
    // Mostrar/ocultar al hacer clic en el cuadro visual
...
// ✅ CAMBIO 1: Permitir guardar con al menos 1 foto (no las 3 obligatorias)
...
const paths = { 
    f: files.f ? `${uid}/${ts}_f.jpg` : null, 
    i: files.i ? `${uid}/${ts}_i.jpg` : null, 
    d: files.d ? `${uid}/${ts}_d.jpg` : null 
};
...
else if (err.message.includes('Debe subir al menos una')) m = err.message; // Mostrar el mensaje personalizado de fotos
mostrarError(m);
} finally {
btn.disabled = false;
