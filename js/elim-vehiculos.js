window.initElimVehiculos = function() {
  ...
  const crossWarning = document.getElementById('cross-plate-warning'); // Si existe en el DOM, si no, se ignora
  ...
  function cargarDatos(data, source) {
    ...
    // 🔔 LÓGICA DE UI SEGÚN ESTADO (Banner de reintegro SOLO aquí, después de seleccionar)
    if (isArchived) {
  ...
  btnEliminar.addEventListener('click', () => {
    if (isSelectionMode) return showMsgElim('⚠️
